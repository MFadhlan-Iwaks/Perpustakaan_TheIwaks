...
switch ($method) {
    case 'GET':
        if ($id_buku) {
            ...
            if ($buku) {
                sendSuccess("Data ditemukan", $buku);
            } else {
    ...
    case 'POST':
        ...
        if (mysqli_query($koneksi, $query)) {
            sendSuccess("Buku ditambahkan", ['id' => mysqli_insert_id($koneksi)], 201);
        } else {
            sendError("Gagal menambahkan buku", 500);
        }
        break;

    case 'PUT':
        if (!$id_buku) {
            sendError("ID diperlukan");
        }
        ...
        if (mysqli_query($koneksi, $query)) {
            sendSuccess("Data buku diperbarui");
        } else {
            sendError("Gagal memperbarui buku", 500);
        }
        break;
