...
switch ($method) {
    case 'GET':
        if ($id_user) {
            ...
            if ($user) sendSuccess("User ditemukan", $user);
            else sendError("User tidak ditemukan", 404);
        } else {
            $query = mysqli_query($koneksi, "SELECT id_user, username, nama_lengkap, role FROM users ORDER BY id_user DESC");
            $users = [];
            while ($row = mysqli_fetch_assoc($query)) { $users[] = $row; }
    ...
    case 'POST': 
        ...
        if (mysqli_num_rows($cek_username) > 0) sendError("Username sudah terdaftar.");

        $query = "INSERT INTO users (username, password, nama_lengkap, role) VALUES ('$username', '$password', '$nama_lengkap', '$role')";
        if (mysqli_query($koneksi, $query)) sendSuccess("User ditambahkan", ['id' => mysqli_insert_id($koneksi)], 201);
    ...
    case 'PUT':
        ...
        if (mysqli_query($koneksi, $query)) sendSuccess("User diperbarui");
        else sendError("Gagal memperbarui user", 500);
        break;

    case 'DELETE':
        if (!$id_user) sendError("ID user diperlukan");
        if (mysqli_query($koneksi, "DELETE FROM users WHERE id_user = $id_user")) sendSuccess("User dihapus");
